...
class UnifiedProgressiveTrainer:
    ...
    def __init__(self, model: SourceFirstDynamicModel, sampler: ViewLocationBalancedSampler, *, pixel_samples: int = 256, learning_rate: float = 1e-3, loss_weights: dict[str, float] | None = None, frequency_bands: dict[str, tuple[float, float]] | None = None, temporal_every: int = 1, temporal_batch_size: int = 8, cardiac_sampling_fraction: float = .8) -> None:
        ...
        self.loss_weights = {"image": 1e-4, "mbc": 1e-5, "smooth": 1e-5, "freq_resp": 1e-4, "freq_card": 1e-4, "score": 1e-5, **(loss_weights or {})}
        self.frequency_bands = frequency_bands or {"respiratory_hz": (.1, .7), "cardiac_hz": (.8, 3.)}
    ...
    def run_stage(self, stage: str, *, steps: int) -> dict[str, object]:
        ...
        for step in range(steps):
            ...
            if stage != "stage1":
                total = total + self.loss_weights["mbc"] * components["mbc"] + self.loss_weights["smooth"] * components["smooth"] + self.loss_weights["score"] * components["score"] + self.loss_weights["freq_resp"] * components.get("freq_resp", total * 0.)
            if stage == "stage3":
                total = total + self.loss_weights["freq_card"] * components.get("freq_card", total * 0.)
            total.backward()
    ...
    def _observation_components(self, observation: DynamicObservation, stage: str) -> dict[str, torch.Tensor]:
        ...
        return {"data": data, "image": image, "mbc": motion["mbc"], "smooth": motion["smooth"], "score": sum(score.square().mean() for score in render["scores"].values())}
    ...
    def _temporal_components(self, stage: str) -> dict[str, torch.Tensor]:
        ...
        if len(sequence) < 3:
            ...
            return {"freq_resp": zero, "freq_card": zero}
        ...
        result = {"freq_resp": frequency_leakage(resp, timestamps, self.frequency_bands["cardiac_hz"]), "freq_card": resp.sum() * 0.}
        if stage == "stage3":
            result["freq_card"] = frequency_leakage(card, timestamps, self.frequency_bands["respiratory_hz"])
        return result
