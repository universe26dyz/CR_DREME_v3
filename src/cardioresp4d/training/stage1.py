...
def _qc_views(model,renderer,uncertainty,root,by_view,ids,patch,out):
    ...
    for view,items in by_view.items():
        values=[]; squared_residuals=[]; pixel_variances=[]; total_variances=[]; observation_variances=[]; slice_mses=[]
        for item in items:
            target,geo=_mean_observation(root,item,patch)
            with torch.no_grad(): q=renderer(model,**geo); pred=q['predicted_slice'].squeeze(); var=uncertainty(q['latent_samples'],torch.tensor([ids[item['mean_slice_id']]]),q['psf_weights'],namespace='mean_slice',enabled=True); t=target.squeeze(); residual=(pred-t).square(); values.append((float(((pred-pred.mean())*(t-t.mean())).mean()/((pred.std()*t.std()).clamp_min(1e-6))),float(torch.sqrt(residual.mean())/(t.max()-t.min()).clamp_min(1e-6)),float(gaussian_nll(pred[None],t[None],var['total_variance'])))); squared_residuals.append(residual.flatten().numpy()); pixel_variances.append(var['pixel_variance'].flatten().numpy()); total_variances.append(var['total_variance'].flatten().numpy()); observation_variances.append(float(var['observation_variance'].mean())); slice_mses.append(float(residual.mean()))
        ...
        array=np.asarray(values); residual=np.concatenate(squared_residuals); pixel=np.concatenate(pixel_variances); total=np.concatenate(total_variances); observation=np.asarray(observation_variances); mse=np.asarray(slice_mses); corr=lambda a,b: float(np.corrcoef(a,b)[0,1]) if len(a)>1 and np.std(a)>0 and np.std(b)>0 else float('nan'); result[view]={'ncc':float(array[:,0].mean()),'nrmse':float(array[:,1].mean()),'loss':float(array[:,2].mean()),'pixel_variance_mean':float(pixel.mean()),'pixel_variance_std':float(pixel.std()),'observation_variance_mean':float(observation.mean()),'observation_variance_std':float(observation.std()),'total_variance_mean':float(total.mean()),'total_variance_std':float(total.std()),'pixel_level_residual_squared_vs_pixel_variance_correlation':corr(residual,pixel),'observation_level_slice_mse_vs_observation_variance_correlation':corr(mse,observation),'normalized_residual_mean':float(np.mean(residual/total))}
    (out/'mean_slice_variance.csv').write_text('view,pixel_variance_mean,observation_variance_mean,total_variance_mean\n'+'\n'.join(f'{v},{m["pixel_variance_mean"]},{m["observation_variance_mean"]},{m["total_variance_mean"]}' for v,m in result.items())+'\n'); return result
...
def main()->None:
    p=argparse.ArgumentParser(description='Run Stage 1 no-motion Canonical INR.'); p.add_argument('--initial-reference',type=Path,required=True);p.add_argument('--initial-reference-valid-mask',type=Path);p.add_argument('--canonical-domain',type=Path,required=True);p.add_argument('--manifest',type=Path,required=True);p.add_argument('--qc-table',type=Path);p.add_argument('--output-dir',type=Path,required=True);p.add_argument('--profile',choices=('smoke','production'),default='production');p.add_argument('--device',choices=('auto','cpu','cuda'),default='auto');p.add_argument('--stage1a-steps',type=int,default=1000);p.add_argument('--stage1a-batch-size',type=int,default=8192);p.add_argument('--stage1a-lr',type=float,default=2e-3);p.add_argument('--stage1b-epochs',type=int,default=30);p.add_argument('--pixels-per-view',type=int,default=2048);p.add_argument('--cardiac-roi-margin-mm',type=float,default=15.);p.add_argument('--roi-sampling-fraction',type=float,default=.8);p.add_argument('--stage1b-lr',type=float,default=1e-3);p.add_argument('--uncertainty-mode',choices=('off','calibrated'),default='off');p.add_argument('--overview-spacing-mm',type=float,default=4.0);p.add_argument('--cardiac-export-spacing-mm',type=float,default=1.5);p.add_argument('--eval-chunk-pixels',type=int,default=8192);p.add_argument('--coarsest-resolution-mm',type=float,default=16.);p.add_argument('--finest-resolution-mm',type=float,default=1.5);p.add_argument('--level-scale',type=float,default=1.38);p.add_argument('--features-per-level',type=int,default=2);p.add_argument('--log2-hashmap-size',type=int,default=19);p.add_argument('--hidden-dim',type=int,default=64);p.add_argument('--latent-dim',type=int,default=32);p.add_argument('--seed',type=int,default=0);args=p.parse_args();kw={k:getattr(args,k) for k in ('coarsest_resolution_mm','finest_resolution_mm','level_scale','features_per_level','log2_hashmap_size','hidden_dim','latent_dim')}; a=train_stage1a(args.initial_reference,args.initial_reference_valid_mask,args.canonical_domain,args.output_dir,steps=args.stage1a_steps,batch_size=args.stage1a_batch_size,lr=args.stage1a_lr,profile=args.profile,device=args.device,seed=args.seed,**kw); mean=build_mean_slices(args.manifest,args.output_dir/'reference',qc_table_path=args.qc_table); b=train_stage1b_balanced(a['checkpoint'],mean,args.canonical_domain,args.output_dir,epochs=args.stage1b_epochs,pixels_per_view=args.pixels_per_view,margin_mm=args.cardiac_roi_margin_mm,roi_fraction=args.roi_sampling_fraction,lr=args.stage1b_lr,seed=args.seed,device=args.device,uncertainty_mode=args.uncertainty_mode); stage1b_metadata=torch.load(b['checkpoint'],map_location='cpu',weights_only=True); qc=run_final_static_qc(_load_checkpoint(Path(a['checkpoint'])),_load_checkpoint(Path(b['checkpoint'])),mean,args.manifest,args.canonical_domain,args.output_dir,overview_spacing_mm=args.overview_spacing_mm,cardiac_export_spacing_mm=args.cardiac_export_spacing_mm,cardiac_roi_margin_mm=args.cardiac_roi_margin_mm,eval_chunk_pixels=args.eval_chunk_pixels,stage1b_metadata=stage1b_metadata); print(json.dumps({'stage1a':a,'stage1b':b,'final_static_qc':qc},indent=2))
