...
class _ScoreWeightedField(nn.Module):
    def __init__(self, mbc: nn.Module, scores: torch.Tensor) -> None:
        super().__init__(); self.mbc = mbc; self.scores = scores

    def forward(self, points_world_mm: torch.Tensor) -> torch.Tensor:
        shape = points_world_mm.shape
        field = self.mbc(points_world_mm.reshape(1, -1, 3))
        if field.ndim == 4: field = field[:, None]
        value = torch.einsum("bla,blnaj->bnj", self.scores, field)
        return value.reshape(*shape[:-1], 3)


class _SequentialPullback(nn.Module):
    def __init__(self, respiratory: nn.Module, cardiac: nn.Module) -> None:
        super().__init__(); self.respiratory = respiratory; self.cardiac = cardiac

    def forward(self, observation_points_mm: torch.Tensor) -> dict[str, torch.Tensor]:
        cardiac_dvf_mm = self.cardiac(observation_points_mm)
        cardiac_points_mm = observation_points_mm + cardiac_dvf_mm
        respiratory_dvf_mm = self.respiratory(cardiac_points_mm)
        return {"cardiac_dvf_mm": cardiac_dvf_mm, "respiratory_dvf_mm": respiratory_dvf_mm, "cardiac_points_mm": cardiac_points_mm, "reference_points_mm": cardiac_points_mm + respiratory_dvf_mm}
...
class SourceFirstDynamicModel(nn.Module):
    ...
    def __init__(self, lower_world_mm: torch.Tensor, upper_world_mm: torch.Tensor, *, cardiac_lower_world_mm: torch.Tensor, cardiac_upper_world_mm: torch.Tensor, n_dynamic_frames: int, inr_width: int = 64, inr_depth: int = 2, latent_dim: int = 16, motion_hidden_dim: int = 64, respiratory_grid_shapes: tuple[tuple[int, int, int], ...] = ((8, 8, 8), (12, 12, 12), (16, 16, 16)), cardiac_grid_shape: tuple[int, int, int] = (16, 16, 16), psf_samples: int = 8) -> None:
        ...
        self.film_encoder = GeometryFiLMMotionEncoder(channels=motion_hidden_dim)
    ...
    def predict(self, observation: DynamicObservation, pixel_uv: torch.Tensor, stage: str) -> dict[str, torch.Tensor]:
        ...
        respiratory = _ScoreWeightedField(self.respiratory_mbc, respiratory_scores)
        cardiac = _ScoreWeightedField(self.cardiac_mbc, cardiac_scores)
        ...
        render = self.psf(self.canonical, points, resolution, motion=_SequentialPullback(respiratory, cardiac))
