...
class SourceFirstDynamicModel(nn.Module):
    ...
    def __init__(self, lower_world_mm: torch.Tensor, upper_world_mm: torch.Tensor, *, cardiac_lower_world_mm: torch.Tensor, cardiac_upper_world_mm: torch.Tensor, n_dynamic_frames: int, inr_width: int = 64, inr_depth: int = 2, latent_dim: int = 16, motion_hidden_dim: int = 64, respiratory_grid_shapes: tuple[tuple[int, int, int], ...] = ((8, 8, 8), (12, 12, 12), (16, 16, 16)), cardiac_grid_shape: tuple[int, int, int] = (16, 16, 16), psf_samples: int = 8, canonical_coarsest_resolution: float = 8., canonical_finest_resolution: float = 1., canonical_level_scale: float = 1.5, canonical_features_per_level: int = 2, canonical_log2_hashmap_size: int = 19, canonical_spatial_scaling: float = 1., respiratory_cps: int | tuple[int, int, int] = 2, cardiac_cps: int | tuple[int, int, int] = 2, cardiac_taper_mm: float = 4., film_channels: int | None = None, film_position_bands: int = 4, uncertainty_frame_embedding_dim: int = 8, uncertainty_width: int | None = None, uncertainty_depth: int | None = None, respiratory_smoothness_shape: tuple[int, int, int] = (16, 16, 16), cardiac_smoothness_shape: tuple[int, int, int] = (16, 16, 16), image_regularization_mode: str = "edge", image_regularization_delta: float = 1.) -> None:
        super().__init__()
